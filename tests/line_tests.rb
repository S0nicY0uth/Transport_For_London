describe Line do
    ...
    describe "routing" do
        ...
        it "should return a list of stations between two points" do
            @victoria_line.route(@oxford_circus, @oval).must_equal([@oxford_circus, @warren_street, @oval])
        end
        it "should return a list of stations between two points in opposite direction" do
            @victoria_line.route(@oval, @oxford_circus).must_equal([@oval, @warren_street, @oxford_circus])
        end
        it "should return the station if the stations are the same" do
            @victoria_line.route(@oval, @oval).must_equal([@oval])
        end
    end
